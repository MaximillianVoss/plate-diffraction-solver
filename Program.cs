...
namespace Diffraction
{
    internal static class Program
    {
        ...
        public class DifrOnLenta
        {
            ...
            {
                double halfL = (b - a) / 2.0, midP = (b + a) / 2.0, tau_x = (x - midP) / halfL;
                if (tau_x < -1) tau_x = -1; if (tau_x > 1) tau_x = 1;
                Compl phi = new Compl(0, 0);
                for (int j = 0; j < N; j++) phi += y[j] * Cheb(j, tau_x);
            ...
            {
                double halfL = (b - a) / 2.0, xi = (2.0 * x - (b + a)) / (b - a);
                double w2 = 1.0 - xi * xi; if (w2 < 1e-10) w2 = 1e-10;
                Compl phi = new Compl(0, 0);
                for (int j = 0; j < N; j++) phi += y[j] * new Compl(ChebAB(j, x));
            ...
            {
                int M = 40;
                double dx = (b - a) / M, sumErr = 0, k_wave = 2 * Math.PI / lambda;
                int count = 0;
                for (int i = 1; i < M; i++)
                {
                    double x = a + i * dx;
            ...
            public double CalculateAbsorbedEnergy()
            {
                ...
                const int M = 200; double dx = (b - a) / M;
                for (int m = 1; m < M; m++)
                {
                    double x = a + m * dx;
